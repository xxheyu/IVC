function [ zero_y, zero_cb, zero_cr, dc_y, dc_cb, dc_cr] = IntraEncodeBlock( block_Y, block_Cb, block_Cr, q_factor,option)
...
dc_cb = zigzag_cb(1);
dc_cr = zigzag_cr(1);
zero_y = ZeroRunEnc(zigzag_y(2:end));
zero_cb = ZeroRunEnc(zigzag_cb(2:end));
zero_cr = ZeroRunEnc(zigzag_cr(2:end));
end